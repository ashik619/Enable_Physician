 ...
 */
public class PendingFragment extends Fragment implements OnTaskCompleted{
    ListView listView;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.pending_fragment_layout, container, false);
    ...
    void getPendingReportsApiCall(){
        ...
        try {
            ...
            URL url = new URL(baseUrl+"/reports/getPendingReportlist");
            ...
            try {
                ...
                if (success) {
                    ...
                    if(msg == 5) {
                        ...
                        RealmResults<PendingReport> results = myRealm.where(PendingReport.class).findAll();
    ...
    void populateListView(){
        RealmResults<PendingReport> ecgReports = myRealm.where(PendingReport.class).findAll();
        if(ecgReports.size()!=0) {
            LocalAdapter adapter = new LocalAdapter(getActivity().getApplicationContext(), ecgReports, true);
            ...
            for (int i = 0; i < result.length(); i++) {
                ...
                myRealm.executeTransaction(new Realm.Transaction() {
                    ...
                    public void execute(Realm realm) {
                        PendingReport pendingReport = realm.createObject(PendingReport.class,report_id);
                        try {

                            pendingReport.setExpert_id(report.getString("expert_id"));
                            pendingReport.setExpertName(report.getString("ExpName"));
                            pendingReport.setImageUrl(report.getString("imgUrl"));
                            pendingReport.setThubUrl(report.getString("thumbUrl"));
                            pendingReport.setAbout_me(report.getString("aboutme"));
                            pendingReport.setLocation(report.getString("location"));
                            pendingReport.setQualification(report.getString("qualification"));
                            populateListView();
    ...
    public class LocalAdapter extends RealmBaseAdapter<PendingReport> implements ListAdapter {
        RelativeLayout btn;
        RealmResults<PendingReport> realmResults;
        public LocalAdapter(Context context, RealmResults<PendingReport> realmResults, boolean automaticUpdate) {
        ...
        public View getView(final int position, View convertView, ViewGroup parent ) {
            ...
            final PendingReport item = realmResults.get(position);
            btn.setTag(position);
            profilePicture.setTag(position);
            tvReportId.setText("ECG#"+item.getReport_id().toString());
            ...
            btn.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    PendingReport item = realmResults.get(position);
